...
with open('skeleton_data.csv', 'w', newline='') as csvfile:
    ...
    header = ['Frame'] + [f'Joint {joint_id+1} X' for joint_id in joint_ids] + [f'Joint {joint_id+1} Y' for joint_id in joint_ids] + [f'Joint {joint_id+1} Z' for joint_id in joint_ids]
        ...
        if tracked_body_index is not None:
            ...
            for connection in bone_connections:
                ...
                if joint1.TrackingState == TrackingState_Tracked and joint2.TrackingState == TrackingState_Tracked:
                    joint1_pos = kinect.body_joint_to_depth_space(joint1)
                    joint2_pos = kinect.body_joint_to_depth_space(joint2)
                    pygame.draw.line(screen, (255, 0, 0), (joint1_pos.x, joint1_pos.y), (joint2_pos.x, joint2_pos.y), 5)
